...
class Aggregrate implements ListenerAggregateInterface
{
    ...
    public function sendEmail($e) {
        ...
        $config = $e->getParam('notification-config');
        ...
        foreach ($config['send-options'] as $key => $value) {
        ...
        $message  = (new Message())
            ->addTo($config['send-options']['to'])
            ->addFrom($config['send-options']['from'])
            ->setSubject($config['send-options']['subject'])
            ->setBody($body)
            ->setEncoding('UTF-8');
        $transport = $this->container->get('notification-transport-file');
